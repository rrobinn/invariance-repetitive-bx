...
# Robin Sifre modified function to that user can input same calibration sample order for reproducibility.
aMNLFA.sample<-function(input.object, ru){
  ...
  if (!is.null(mytime)){
    ...
    # RDS added so that the same sample is used for each latent var
    #ru = stats::runif(dim(mrdata)[1], min = 0, max = 1)
    ##ru = data.frame(ru)
    #write.csv(ru, '/Users/sifre002/Box/sifre002/7_Rscripts/LongitudinalRBS/aMNLFA/calibsample.csv' )
    #ru= read.csv('/Users/sifre002/Box/sifre002/7_Rscripts/LongitudinalRBS/aMNLFA/Invariance_Age_Only/calib_sample2/calibsample.csv')
    mrdata$ranuni = ru$ru
  ...
  write.csv(varlist, file = paste(dir, '/vars.csv', sep =''))
