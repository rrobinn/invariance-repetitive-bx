...
aMNLFA.final<-function(input.object){
  ...
  ##Read in output and test for sig effects, based on bh
  ...
  #meanimpact$pval = p.adjust(meanimpact$pval, method = 'BH', n = length(meanimpact$pval))
  mean_totest<-meanimpact
  #mean_totest$sig = NA
  #mean_totest$sig = ifelse(mean_totest$pval<.05, "TRUE", "FALSE")
  #test = meanimpact$pval 
  #i/(m*5)
  
  meanimpact<-meanimpact[which(meanimpact$pval<.05),]
  ...
  if (nrow(lambdaconstraints)==0) {
    lambdaconstraints[1,] <- NA
    ...
    for (i in 1:length(lambdaconstraints)){
      for (r in 1:length(myindicators)){
        for (c in 1:length(myMeasInvar)){
          # alllambdadf[r,c]<- ifelse(dim(lambdaconstraints)[1]==0, NA, ifelse(item_i[i]==r & col_i[i]==c,myMeasInvar[c],NA) ) RDS changed this 
          alllambdadf[r,c]<- ifelse(length(lambdaconstraints)==0, NA, ifelse(item_i[i]==r & col_i[i]==c,myMeasInvar[c],NA) )
        }}}}
  ...
  if (nrow(interceptconstraints)== 0)  {interceptconstraints[1,] = NA}
  ...
  m<-mitem*length(myMeasInvar) # for mean, m = mitem
  int_totest$sig<-NA
  for (l in 1:dim(int_totest)[1]){ # for every DEF
