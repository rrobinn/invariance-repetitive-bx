...
homedir <- '/Users/sifre002/Box/sifre002/18-Organized-Code/invariance-repetitive-bx/'
calib_sample = '1'
subscale = 'REP'
wd = paste(homedir, 'output/calib_', calib_sample,'/', subscale, '/', sep ='')


# Read in cleaned RBS-EC data 
df <- read.csv(paste(homedir, 'data/RBSEC.csv', sep = ''),header=TRUE)
...
source(paste(homedir, 'MNLFA/scripts/aMNLFA_sample.R', sep='')) 
aMNLFA.sample(ob,ru)
...
####################################################
source(paste(homedir, 'MNLFA/scripts/aMNLFA_initial.R', sep ='')) 
aMNLFA.initial(ob) 
# Run Models 
...
################################
source(paste(homedir, 'MNLFA/scripts/aMNLFA_simultaneous.R', sep =''))
...
source(paste(homedir,'MNLFA/scripts/aMNLFA_scoreplots.R', sep = ''))
