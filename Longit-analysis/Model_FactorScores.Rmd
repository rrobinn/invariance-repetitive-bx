...
myaicc #We sekect linear fixed effect. dAICc does not justify quadratic term
...
myaicc  <- ICtab(logLik(glm.1),
                 ...
                 weights =TRUE,delta =TRUE,sort=TRUE,mnames=mynames,base=TRUE)
myaicc$rweight <- round(max(myaicc$weight) / myaicc$weight,2)
myaicc # Adding in RE improves model fit 
...
logit2prob(coef(glm.3$AGE18))
# type ='response' gives response on natural scale 
dat$rep_ymean = predict(glm.3, newdat = dat, type = 'response')   # gives probability  (can confirm using function above )
test = predict(glm.3, newdat = dat)
...
myaicc  <- ICtab(logLik(glm.0),
                 ...
                 nobs = nrow(na.omit(dat)),
                 weights =TRUE,delta =TRUE,sort=TRUE,mnames=mynames,base=TRUE)
myaicc # Adding linear effect of age improves model fit 
...
myaicc  <- ICtab(logLik(glm.1),
                 ...
                 nobs = nrow(na.omit(dat)),
                 weights =TRUE,delta =TRUE,sort=TRUE,mnames=mynames,base=TRUE)
myaicc # Adding random effect of age does not improve model fit 
...
mynames <- as.character (c("glm.1", "glm.2", "glm.3")) # model numbers as characters
myaicc  <- ICtab(logLik(glm.1),
                 logLik(glm.2),
                 logLik(glm.3),
                 type ="AICc",
                 nobs = nrow(na.omit(dat)),
                 weights =TRUE,delta =TRUE,sort=TRUE,mnames=mynames,base=TRUE)
myaicc # glm.3 (w/ tablet covariate has slightly better fit. we select this model bc dAICc is higher)
...
dat$self_yhat_mean = predict(glm.3, newdat = dat, type = 'response')   # gives probability (confirmed)
...
myaicc  <- ICtab(logLik(glm.0),
                 ...
                 weights =TRUE,delta =TRUE,sort=TRUE,mnames=mynames,base=TRUE)
myaicc
summary(glm.0) # We adopt intercept-only model
